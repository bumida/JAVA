	...
	<c:set var="replyBeanList" value="${requestScope.replyBeanList}"/>
		...
		<c:choose>
			...
			<c:forEach var="reply" items="${replyBeanList}">
				...
				<tr>
					<td align="center" width="130px">${reply.getMember_id()}</td>
					<td valign="top" style="padding-left:10px;">
						<textarea id="${i}" name="replys${i}" class="replys" style="width:750px; resize:none" readonly>${reply.getReply_contents()}</textarea>
						<c:if test="${reply.getMember_id() eq session_id}">
							<a id="ready${i}" style="display:inline" href="javascript:updateReply(${i})">[수정]</a>
							<a id="ok${i}" style="display:none" href="javascript:submitReplyUpdateForm(${reply.getReply_num()}, ${reply.getBoard_num()}, ${i})">[수정 완료]</a>
